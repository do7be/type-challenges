...
type Sum<A extends string | number | bigint, B extends string | number | bigint, AD extends string = ReverseString<`${A}`>, BD extends string = ReverseString<`${B}`>> = ReverseString<SumForLoop<AD, BD>> extends infer R extends string
? R extends ''
  ? '0'
  : R
: never

type MultipleLoop<A extends string, B extends string, I extends any[] = [], R extends string = '0'> = `${I['length']}` extends B
? R
: MultipleLoop<A, B, [...I, any], Sum<R, A>>

type Multiple<A extends string, B extends string, R extends string = '0'> = B extends `${infer B0}${infer BR}`
? Multiple<`${A}0`, BR, Sum<R, MultipleLoop<A, B0>>>
: R

type Multiply<A extends number | string | bigint, B extends number | string | bigint> = `${A}` extends '0'
? '0'
: Multiple<`${A}`, ReverseString<`${B}`>>
